...
def ComputeMbmpPlus(
    Target: np.ndarray,
    Reference: np.ndarray,
    PlumeMask: np.ndarray,
    RidgeAlpha: float = 1.0,
    Epsilon: float = 1e-6,
    MinimumBackgroundPixels: int = 50,
) -> np.ndarray:
    """
    Calcula MBMPPlus con corrección supervisada de fondo.

    Procedimiento:
    1. Calcula MBMP inverso.
    2. Calcula ratios temporales Bk_target / Bk_reference para todas las bandas excepto B11 y B12.
    3. Define fondo con la máscara ground truth: background = plume == 0.
    4. Estandariza ratios usando solo píxeles de fondo.
    5. Ajusta Ridge para predecir el componente de ruido/fondo en MBMP inverso.
    6. MBMPPlus = MBMP_inverse - predicted_background_noise.

    Esta versión usa PlumeMask y por tanto debe auditarse como:
    ...
    if Target.shape != Reference.shape:
        raise ValueError(f"Target y Reference deben tener la misma forma. {Target.shape} vs {Reference.shape}")

    Bands, Height, Width = Target.shape
    Mask = ValidateMask(PlumeMask, ExpectedShape=(Height, Width), Name="PlumeMask")

    BackgroundPixels = (~Mask).reshape(-1)

    if int(BackgroundPixels.sum()) < MinimumBackgroundPixels:
        raise ValueError(
            f"No hay suficientes píxeles de fondo para MBMPPlus. "
            f"BackgroundPixels={int(BackgroundPixels.sum())}, "
            f"Minimum={MinimumBackgroundPixels}"
        )
    ...
    ExcludedBands = {S2_BAND_INDEX["B11"], S2_BAND_INDEX["B12"]}
    ...
    Predictors = TemporalRatios[PredictorBandIndices].T.astype(np.float32)

    ValidPixels = np.isfinite(MbmpInverse)
    ValidPixels &= np.isfinite(Predictors).all(axis=1)

    ValidBackground = ValidPixels & BackgroundPixels

    if int(ValidBackground.sum()) < MinimumBackgroundPixels:
        raise ValueError(
            f"No hay suficientes píxeles válidos de fondo para Ridge. "
            f"ValidBackground={int(ValidBackground.sum())}, "
            f"Minimum={MinimumBackgroundPixels}"
        )

    BackgroundPredictors = Predictors[ValidBackground]

    Mean = np.nanmean(BackgroundPredictors, axis=0)
    Std = np.nanstd(BackgroundPredictors, axis=0)
    Std = np.where(Std < Epsilon, 1.0, Std)

    PredictorsZ = (Predictors - Mean) / Std

    RidgeModel = Ridge(alpha=RidgeAlpha, fit_intercept=True)
    RidgeModel.fit(PredictorsZ[ValidBackground], MbmpInverse[ValidBackground])

    PredictedNoise = np.full_like(MbmpInverse, fill_value=np.nan, dtype=np.float32)
    PredictedNoise[ValidPixels] = RidgeModel.predict(PredictorsZ[ValidPixels]).astype(np.float32)

    MbmpPlus = np.full_like(MbmpInverse, fill_value=np.nan, dtype=np.float32)
    MbmpPlus[ValidPixels] = MbmpInverse[ValidPixels] - PredictedNoise[ValidPixels]

    MbmpPlusImage = MbmpPlus.reshape(Height, Width)

    if not np.isfinite(MbmpPlusImage).any():
        raise ValueError("MBMPPlus no contiene valores finitos.")

    FillValue = float(np.nanmedian(MbmpPlusImage[np.isfinite(MbmpPlusImage)]))
    MbmpPlusImage = np.where(np.isfinite(MbmpPlusImage), MbmpPlusImage, FillValue)

    return MbmpPlusImage.astype(np.float32)
...
# ============================================================
# MBMPPlus UNSUPERVISED OVERRIDE
# ============================================================
# This block intentionally overrides any previous supervised
# ComputeMbmpPlus / ComputeMBMPPlus implementation.
#
# Methodological decision:
# - MBMPPlus must not use Plume / GroundTruth.
# - MBMPPlus must be reproducible at inference time.
# - Any extra positional argument passed by old code is ignored.
# ============================================================

def RidgeClean2D(Image, Sigma=1.0):
    """
    Limpieza no supervisada de MBMP.

    No usa máscara plume, ground truth ni píxeles de fondo definidos por etiqueta.
    """
    import numpy as np
...
def ComputeMbmpPlus(TargetB12, ReferenceB12, *args, Sigma=1.0, **kwargs):
    ...
    Esta versión NO usa Plume ni GroundTruth.
    Si código antiguo pasa Plume como tercer argumento, se ignora.
